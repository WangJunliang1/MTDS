...
# First Party
from MTDS.config import (
    MTDSEngineConfig,
    MTDSEngineMetadata,
    MTDSMemPoolMetadata,
)
from MTDS.logging import init_logger
from MTDS.storage_backend.abstract_backend import LMCBackendInterface
from MTDS.storage_backend.hybrid_backend import (  # , LMCPipelinedHybridBackend
    LMCHybridBackend,
)
...
def CreateStorageBackend(
    ...
    mpool_metadata = MTDSMemPoolMetadata(
        metadata.kv_shape, metadata.kv_dtype, config.max_local_cache_size
    )
    match config:
        case MTDSEngineConfig(_, local_device=None, remote_url=str(p)) if (
            p is not None
        ):
            # remote only
        ...
        case MTDSEngineConfig(_, local_device=str(p), remote_url=None) if (
            p is not None
        ):
            # local only
        ...
        case MTDSEngineConfig(_, local_device=str(p), remote_url=str(q)) if (
