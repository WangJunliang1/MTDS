...
from MTDS.v1.cache_engine import MTDSEngine, MTDSEngineBuilder
from MTDS.v1.config import MTDSEngineConfig
from MTDS.v1.gpu_connector import (
...
# from MTDS_vllm.blend_adapter import remove_request_id_indices

logger = init_logger(__name__)

MTDS_CUDA_STREAM = torch.cuda.Stream()
...
def need_gpu_interm_buffer(MTDS_config: MTDSEngineConfig):
    if MTDS_config.enable_nixl:
        return False
    else:
        return True


def init_MTDS_engine(
    model_config: ModelConfig,
    parallel_config: ParallelConfig,
    cache_config: CacheConfig,
    scheduler_config: SchedulerConfig,
) -> Optional[MTDSEngine]:
    """Initialize the MTDS engine by the given model config and parallel
    config. This function will check the environment variable
    `MTDS_CONFIG_FILE` to load the configuration file. If that environment
    ...
    :return: The initialized MTDS engine or None (if the environment variable
        `MTDS_CONFIG_FILE` is not set).
    :rtype: Optional[MTDSEngine]
    """
    if MTDSEngineBuilder.get(ENGINE_NAME) is not None:
        return None
    ...
    assert isinstance(config, MTDSEngineConfig), (
        "MTDS v1 configuration is should be passed."
    )
    ...
    engine = MTDSEngineBuilder.get_or_create(
...
def close_MTDS_engine() -> None:
    """Close the MTDS engine if it is initialized."""
    logger.debug("Closing MTDS Engine")
    MTDSEngineBuilder.destroy(ENGINE_NAME)


# This function is not used for now
def MTDS_should_retrieve(
    model_input: "ModelInputForGPUWithSamplingMetadata",
) -> List[RetrieveStatus]:
    """Check should we retrieve KV from MTDS for the current model_input.
    ...
    has_engine = MTDSEngineBuilder.get(ENGINE_NAME) is not None
    if not has_engine:
...
def MTDS_should_store(
    model_input: "ModelInputForGPUWithSamplingMetadata",
) -> List[StoreStatus]:
    """Check should we store KV into MTDS for the current model_input.
    ...
    engine = MTDSEngineBuilder.get(ENGINE_NAME)
    ...
    # doesn't need to be done twice in `MTDS_retrieve` and
    # `MTDS_store`
    ...
    for seq_group_idx, seq_group in enumerate(seq_group_list):
        ...
        # `MTDS_model_request` inside `cache_engine`
        # Check whether user has specified to not store the cache
        if hasattr(seq_group, "MTDS_model_request"):
            MTDS_model_request = seq_group.MTDS_model_request
            if MTDS_model_request is not None:
                user_should_store = MTDS_model_request.store_cache
...
def MTDS_store_kv(
    ...
    """Store the KV caches into MTDS for the current model_input.
    ...
    """
    engine = MTDSEngineBuilder.get(ENGINE_NAME)
    assert engine is not None, "MTDS engine is not initialized."
...
def MTDS_retrieve_kv(
    ...
    """Retrieve the KV caches from MTDS for the current model_input. And
