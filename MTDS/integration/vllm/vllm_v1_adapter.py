...
# First Party
from MTDS.integration.vllm.utils import ENGINE_NAME, MTDS_get_config
from MTDS.integration.vllm.vllm_adapter import init_MTDS_engine
from MTDS.logging import init_logger
from MTDS.utils import _MTDS_nvtx_annotate
from MTDS.v1.cache_engine import LayerwiseMTDSEngine, MTDSEngine
from MTDS.v1.compute.blend import LMCBlenderBuilder
...
def get_zmq_rpc_path_MTDS(
    ...
    return f"ipc://{base_url}/MTDS_rpc_port_{rpc_port}"


# TODO: move this to MTDS so that we can gracefully close it
class MTDSLookupClient:
    def __init__(self, role: KVConnectorRole, is_tp: bool, vllm_config: "VllmConfig"):
        self.encoder = MsgpackEncoder()
        self.ctx = zmq.Context()  # type: ignore[attr-defined]
        socket_path = get_zmq_rpc_path_MTDS(role, is_tp, vllm_config)
    ...
    def __init__(
        ...
        def process_request():
            while self.running:
                ...
                result = self.MTDS_engine.lookup(token_ids, pin=True)
                response = result.to_bytes(4, "big")
                self.socket.send(response)
                # except Exception as e:
                #    logger.error("Error in MTDS lookup server: %s", e)
                #    break
                # continue
...
class LoadSpec:
    ...
    # Number of tokens that are cached in MTDS
    MTDS_cached_tokens: int
...
class ReqMeta:
    ...
    def from_request_tracker(
        ...
        Args:
            ...
            MTDS_chunk_size (int): the chunk size for MTDS.
        ...
        chunk_boundary = (
            cdiv(tracker.num_saved_tokens + 1, MTDS_chunk_size) * MTDS_chunk_size
        )
        ...
        # setting
        num_tokens_to_save = (
            (input_token_len // MTDS_chunk_size * MTDS_chunk_size)
...
@dataclass
class MTDSConnectorMetadata(KVConnectorMetadata):
...
class MTDSConnectorV1Impl:
    def __init__(
        ...
        config = MTDS_get_config()

        if role == KVConnectorRole.SCHEDULER:
            self.lookup_client = MTDSLookupClient(role, is_tp, vllm_config)
        else:
            self.MTDS_engine = init_MTDS_engine(
            ...
            if self.enable_blending:
                self.blender = LMCBlenderBuilder.get_or_create(
                    ENGINE_NAME,
                    self.MTDS_engine,
                    self.MTDS_engine.gpu_connector,
                )

            # NOTE: Only create the KV lookup API server on worker rank 0
            # when there are multiple workers
            assert self.MTDS_engine is not None
            if vllm_config.parallel_config.rank == 0:
                self.lookup_server = MTDSLookupServer(
                    self.MTDS_engine, role, is_tp, vllm_config
                )
        ...
        # request_id -> (vllm cached tokes, MTDS cached tokens)
        ...
        self._MTDS_chunk_size = config.chunk_size
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs) -> None:
        ...
        assert isinstance(metadata, MTDSConnectorMetadata)
        ...
        for idx, request in enumerate(metadata.requests):
            ...
            if self.use_layerwise:
                assert isinstance(self.MTDS_engine, LayerwiseMTDSEngine)
                if idx == last_idx:
                    sync = True
                else:
                    sync = False
                ...
                if self.enable_blending:
                    self.blender.blend(
                        tokens[: request.load_spec.MTDS_cached_tokens],
                        token_mask[: request.load_spec.MTDS_cached_tokens],
                        kvcaches=kvcaches,
                        slot_mapping=slot_mapping,
                    )
                else:
                    # TODO(Jiayi): Need to make prefix caching and blending compatible
                    layerwise_retriever = self.MTDS_engine.retrieve_layer(
                        tokens,
                        token_mask,
                        kvcaches=kvcaches,
                        slot_mapping=slot_mapping,
                ...
                ret_token_mask = self.MTDS_engine.retrieve(
                    tokens,
                    token_mask,
                    kvcaches=kvcaches,
                    slot_mapping=slot_mapping,
                ...
                num_expected_tokens = (
                    request.load_spec.MTDS_cached_tokens
                    - request.load_spec.vllm_cached_tokens
    ...
    def save_kv_layer(
        ...
        **kwargs,
        ...
        assert isinstance(connector_metadata, MTDSConnectorMetadata)
        ...
        if self.current_layer == 0:
            ...
            for idx, request in enumerate(connector_metadata.requests):
                ...
                # NOTE: In PD setting, MTDS_engine.lookup() will always
                ...
                #    self.MTDS_engine.lookup(token_ids),
                ...
                if not is_first:
                    ...
                    sync = False
                layerwise_storer = self.MTDS_engine.store_layer(
                    token_ids,
                    mask=store_mask,
                    kvcaches=kvcaches,
                    slot_mapping=slot_mapping,
    ...
    def wait_for_save(self):
        ...
        for request in connector_metadata.requests:
            ...
            # TODO: have a pre-allocated buffer to hold the slot_mappings
            slot_mapping = slot_mapping.cuda()
            # NOTE: In PD setting, MTDS_engine.lookup() will always return
    ...
    def get_num_new_matched_tokens(
        ...
        logger.info(
            "Reqid: %s, Total tokens %d, MTDS hit tokens: %d, need to load: %d",
            request.request_id,
            request.num_tokens,
        ...
        self.load_specs[request.request_id] = LoadSpec(
            ...
            MTDS_cached_tokens=num_external_hit_tokens,
            can_load=False,
        )
    ...
    def update_state_after_alloc(self, request: "Request", num_external_tokens: int):
        ...
        assert (
            ...
            == self.load_specs[request.request_id].MTDS_cached_tokens
            - self.load_specs[request.request_id].vllm_cached_tokens
        ), (
            f"Mismatch in number of tokens: {num_external_tokens} vs "
            f"{self.load_specs[request.request_id].MTDS_cached_tokens} - "
