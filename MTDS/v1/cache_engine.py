...
from MTDS.observability import MTDSStatsLogger, LMCStatsMonitor
from MTDS.usage_context import InitializeUsageContext
from MTDS.utils import CacheEngineKey, _MTDS_nvtx_annotate
from MTDS.v1.config import MTDSEngineConfig
from MTDS.v1.distributed_server import (
...
from MTDS.v1.lookup_server import LookupServerInterface, RedisLookupServer
from MTDS.v1.memory_management import AdHocMemoryAllocator  # noqa: E501
from MTDS.v1.memory_management import CuFileMemoryAllocator  # noqa: E501
from MTDS.v1.memory_management import (
    MemoryAllocatorInterface,
    MemoryFormat,
    MixedMemoryAllocator,
)
from MTDS.v1.storage_backend.storage_manager import (
    DistributedStorageManager,
    StorageManager,
)
from MTDS.v1.token_database import (
...
class MTDSEngine:
    ...
    def __init__(
        ...
        logger.info(f"Creating MTDSEngine with config: {config}")
        self.config = config
        ...
        # First Party
        from MTDS.v1.cache_controller import MTDSWorker

        self.MTDS_worker: Optional[MTDSWorker] = None
        if self.config.enable_controller:
            self.MTDS_worker = MTDSWorker(config, metadata, self)
    ...
    def close(self) -> None:
        ...
        if self.MTDS_worker is not None:
            self.MTDS_worker.close()

        self.storage_manager.close()
        logger.info("MTDSEngine closed.")


# TODO(Jiayi): Using a separate class here.
# Should use the same class once the code is stable.
class LayerwiseMTDSEngine(MTDSEngine):
    """A specialized MTDSEngine for layerwise cache engine.

    This class is used to store the layerwise cache engine. It is a
    subclass of MTDSEngine and inherits all the methods and attributes
...
class MTDSEngineBuilder:
    _instances: Dict[str, MTDSEngine] = {}
    _cfgs: Dict[str, MTDSEngineConfig] = {}
    _metadatas: Dict[str, MTDSEngineMetadata] = {}
    _stat_loggers: Dict[str, MTDSStatsLogger] = {}
    ...
    def get_or_create(
        ...
        """
        Builds a new MTDSEngine instance if it doesn't already exist for the
        given ID.

        raises: ValueError if the instance already exists with a different
            configuration.
        """
        logger.info(f"Creating MTDSEngine instance {instance_id}")
        if instance_id not in cls._instances:
            memory_allocator = cls._Create_memory_allocator(config, metadata)
            token_database = cls._Create_token_database(config, metadata)
            stat_logger = MTDSStatsLogger(metadata, log_interval=10)

            # HACK(Jiayi): Merge two types of engine into one in the future
            engine: Union[LayerwiseMTDSEngine, MTDSEngine]
            if config.use_layerwise:
                engine = LayerwiseMTDSEngine(
                    config,
                    metadata,
                    memory_allocator,
                    token_database,
    ...
    @classmethod
    def get(cls, instance_id: str) -> Optional[MTDSEngine]:
        """Returns the MTDSEngine instance associated with the instance ID,
        or None if not found."""
        return cls._instances.get(instance_id)

    @classmethod
    def destroy(cls, instance_id: str) -> None:
        """Close and delete the MTDSEngine instance by the instance ID"""
