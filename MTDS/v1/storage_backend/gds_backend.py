...
from MTDS.v1.memory_management import MemoryAllocatorInterface, MemoryObj
...
class GdsBackend(StorageBackendInterface):
    ...
    `gds_path` and `cufile_buffer_size` in their MTDS config.
    ...
    def __init__(
        ...
        self.stats = None  # TODO: plug into MTDS Statistics
    ...
    def _read_metadata(self, key, filename, subdir_key):
        ...
        if extra_metadata["MTDS_version"] != str(_METADATA_VERSION):
            raise RuntimeError("unhandled MTDS metadata")
    ...
    def _save_gds_cufile(
        ...
        metadata = pack_metadata(kv_chunk, MTDS_version=str(_METADATA_VERSION))
        try:
