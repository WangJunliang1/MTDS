...
import MTDS.config as orig_config
...
class MTDSEngineConfig:
    ...
    # the id of the MTDS instance
    MTDS_instance_id: str = "MTDS_default_instance"
    # controller url
    controller_url: Optional[str] = None
    # MTDS worker url
    # NOTE: port number will add `worker_id`
    MTDS_worker_port: Optional[int] = None
    ...
    # The url of the actual remote MTDS instance for auditing
    ...
    @staticmethod
    def from_file(file_path: str) -> "MTDSEngineConfig":
        """
        ...
        MTDS_instance_id = config.get(
            "MTDS_instance_id", "MTDS_default_instance"
        )
        controller_url = config.get("controller_url", None)
        MTDS_worker_port = config.get("MTDS_worker_port", None)
    ...
    def from_env() -> "MTDSEngineConfig":
        ...
        The environment variables should starts with MTDS and be in
        uppercase. For example, `MTDS_CHUNK_SIZE`.
        :note: the default configuration only uses cpu
        """

        def get_env_name(attr_name: str) -> str:
            return f"MTDS_{attr_name.upper()}"
        ...
        config = MTDSEngineConfig.from_defaults(remote_url=None, remote_serde=None)
        ...
        MTDS_instance_id = parse_env(
            get_env_name("MTDS_instance_id"), "MTDS_default_instance"
        )
        assert MTDS_instance_id is not None
        config.MTDS_instance_id = MTDS_instance_id
        config.controller_url = parse_env(
            get_env_name("controller_url"), config.controller_url
        )
        config.MTDS_worker_port = to_int(
            parse_env(get_env_name("MTDS_worker_port"), config.MTDS_worker_port)
        )
        ...
        if config.nixl_receiver_host is None:
            ...
            if config.nixl_receiver_host is not None:
                logger.warning(
                    "MTDS_NIXL_PEER_HOST is deprecated, please use "
                    "MTDS_NIXL_RECEIVER_HOST environment variable instead"
                )
        ...
        if config.nixl_receiver_port is None:
            ...
            if config.nixl_receiver_port is not None:
                logger.warning(
                    "MTDS_NIXL_PEER_PORT is deprecated, please use "
                    "MTDS_NIXL_RECEIVER_PORT environment variable instead"
                )
    ...
    def to_original_config(self) -> orig_config.MTDSEngineConfig:
        # NOTE: This function is purely for UsageContext compatibility
        return orig_config.MTDSEngineConfig(
    ...
    def validate(self) -> "MTDSEngineConfig":
        """Validate the config"""
        if self.enable_p2p:
    ...
    def log_config(self) -> "MTDSEngineConfig":
        """log the configuration in MTDS"""
        config_dict = {
            "chunk_size": self.chunk_size,
            ...
            "MTDS_instance_id": self.MTDS_instance_id,
        ...
        logger.info(f"MTDS Configuration: {config_dict}")
