...
from MTDS.v1.cache_controller.controller_manager import MTDSControllerManager
from MTDS.v1.cache_controller.message import (  # noqa: E501
    CheckFinishMsg,
    CheckFinishRetMsg,
    ClearMsg,
    ClearRetMsg,
...
def create_app(controller_url: str) -> FastAPI:
    """
    Create a FastAPI application with endpoints for MTDS operations.
    """
    MTDS_controller_manager = MTDSControllerManager(controller_url)

    @asynccontextmanager
    async def lifespan(app: FastAPI):
        # Start background task here
        MTDS_cluster_monitor_task = asyncio.create_task(
            MTDS_controller_manager.start_all()
        )
        yield
        # Optionally cancel the task on shutdown
        MTDS_cluster_monitor_task.cancel()
        try:
            await MTDS_cluster_monitor_task
    ...
    async def query_instance(req: QueryInstRequest):
        try:
            ...
            ret_msg = await MTDS_controller_manager.handle_orchestration_message(msg)
...
def main():
    ...
    try:
        ...
        logger.info(f"Starting MTDS controller at {args.host}:{args.port}")
        logger.info(f"Monitoring MTDS workers at port {args.monitor_port}")
