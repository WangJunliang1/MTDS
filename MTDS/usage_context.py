...
from MTDS.connections import global_http_connection
...
class UsageContext:
    ...
    def send_message_server(self, msg, message_type):
        ...
        try:
            ...
            logger.debug("Unable to send MTDS context message")
            ...
            with open(path, "r") as f:
                for line in f:
                    if "docker" in line:
                        return "DOCKER"
        try:
            _ = pkg_resources.get_distribution("MTDS")
            return "PIP"
        except pkg_resources.DistributionNotFound:
            pass
        try:
            result = subprocess.run(
                ["conda", "list", "MTDS"], capture_output=True, text=True
            )
            if "MTDS" in result.stdout:
                return "CONDA"
...
def InitializeUsageContext(
    ...
    server_url = "http://stats.MTDS.ai:8080/endpoint"
    if os.getenv("MTDS_TRACK_USAGE") == "false":
        return None
    else:
