...
def build_format(color):
    ...
    underline = "\x1b[3m"
    return (
        f"{color}[%(asctime)s] MTDS %(levelname)s:{reset} %(message)s "
...
def get_log_level() -> int:
    """
    Try to read MTDS_LOG_LEVEL from environment variables.
    Could be:
    - DEBUG
    - INFO
    - WARNING
    ...
    """
    log_level = os.getenv("MTDS_LOG_LEVEL", "INFO").upper()
...
#
# logging.basicConfig(
#    format="\033[33m%(levelname)s MTDS: \033[0m%(message)s "
