...
from MTDS.utils import thread_safe
...
class LMCStatsMonitor:
    ...
    @thread_safe
    def get_stats_and_clear(self) -> MTDSStats:
        """
...
class PrometheusLogger:
    ...
    def __init__(self, metadata: MTDSEngineMetadata):
        ...
        self.counter_num_retrieve_requests = self._counter_cls(
            name="MTDS:num_retrieve_requests",
            documentation="Total number of retrieve requests sent to MTDS",
            labelnames=labelnames,
        )

        self.counter_num_store_requests = self._counter_cls(
            name="MTDS:num_store_requests",
            documentation="Total number of store requests sent to MTDS",
            labelnames=labelnames,
        )

        self.counter_num_requested_tokens = self._counter_cls(
            name="MTDS:num_requested_tokens",
            documentation="Total number of tokens requested from MTDS",
            labelnames=labelnames,
        )

        self.counter_num_hit_tokens = self._counter_cls(
            name="MTDS:num_hit_tokens",
            documentation="Total number of tokens hit in MTDS",
            labelnames=labelnames,
        )

        self.counter_num_remote_read_requests = self._counter_cls(
            name="MTDS:num_remote_read_requests",
            documentation="Total number of requests read from "
            "remote backends in MTDS",
            labelnames=labelnames,
        )

        self.counter_num_remote_read_bytes = self._counter_cls(
            name="MTDS:num_remote_read_bytes",
            documentation="Total number of bytes read from remote backends in MTDS",
            labelnames=labelnames,
        )

        self.counter_num_remote_write_requests = self._counter_cls(
            name="MTDS:num_remote_write_requests",
        ...
        self.counter_num_remote_write_bytes = self._counter_cls(
            name="MTDS:num_remote_write_bytes",
            documentation="Total number of bytes write to remote backends in MTDS",
            labelnames=labelnames,
        )

        self.gauge_cache_hit_rate = self._gauge_cls(
            name="MTDS:cache_hit_rate",
            documentation="Cache hit rate of MTDS since last log",
            labelnames=labelnames,
            multiprocess_mode="livemostrecent",
        )

        self.gauge_local_cache_usage = self._gauge_cls(
            name="MTDS:local_cache_usage",
            documentation="Local cache usage (bytes) of MTDS",
            labelnames=labelnames,
            multiprocess_mode="sum",
        )

        self.gauge_remote_cache_usage = self._gauge_cls(
            name="MTDS:remote_cache_usage",
            documentation="Remote cache usage (bytes) of MTDS",
            labelnames=labelnames,
            multiprocess_mode="sum",
        )

        self.gauge_local_storage_usage = self._gauge_cls(
            name="MTDS:local_storage_usage",
            documentation="Local storage usage (bytes) of MTDS",
            labelnames=labelnames,
            multiprocess_mode="sum",
        )
        ...
        self.histogram_time_to_retrieve = self._histogram_cls(
            name="MTDS:time_to_retrieve",
            documentation="Time to retrieve from MTDS (seconds)",
            labelnames=labelnames,
        ...
        self.histogram_time_to_store = self._histogram_cls(
            name="MTDS:time_to_store",
            documentation="Time to store to MTDS (seconds)",
            labelnames=labelnames,
        ...
        self.histogram_retrieve_speed = self._histogram_cls(
            name="MTDS:retrieve_speed",
            documentation="Retrieve speed of MTDS (tokens per second)",
            labelnames=labelnames,
        ...
        self.histogram_store_speed = self._histogram_cls(
            name="MTDS:store_speed",
            documentation="Store speed of MTDS (tokens per second)",
            labelnames=labelnames,
        ...
        self.histogram_remote_time_to_get_sync = self._histogram_cls(
            name="MTDS:remote_time_to_get_sync",
    ...
    def log_prometheus(self, stats: MTDSStats):
        self._log_counter(
    ...
    @staticmethod
    def _metadata_to_labels(metadata: MTDSEngineMetadata):
        return {
    ...
    @staticmethod
    def GetOrCreate(metadata: MTDSEngineMetadata) -> "PrometheusLogger":
...
class MTDSStatsLogger:
    def __init__(self, metadata: MTDSEngineMetadata, log_interval: int):
        self.metadata = metadata
