...
def lmserver_v1_process(request):
    ...
    while max_retries > 0:
        ...
        print("Starting the MTDS v1 server process on port")
        proc = subprocess.Popen(
            shlex.split(
                f"python3 -m MTDS.v1.server localhost {port_number} {device}"
            )
        )

        # Wait for MTDS process to start
        time.sleep(5)
    ...
    yield MTDSServerProcess(server_url, proc)
...
def lmserver_process(request):
    ...
    while max_retries > 0:
        max_retries -= 1
        port_number = random.randint(10000, 65500)
        print("Starting the MTDS server process on port")
        proc = subprocess.Popen(
            shlex.split(f"python3 -m MTDS.server localhost {port_number} {device}")
        )
