...
# First Party
from MTDS.cache_engine import MTDSEngine, MTDSEngineBuilder
from MTDS.config import MTDSEngineConfig, MTDSEngineMetadata


def dumb_metadata(fmt="vllm", kv_shape=(32, 2, 256, 8, 128)):
    return MTDSEngineMetadata("test_model", 3, 123, fmt, torch.bfloat16, kv_shape)
...
def test_retrieve_device(backend, src_device, dst_device, autorelease):
    ...
    cfg = MTDSEngineConfig.from_legacy(chunk_size=chunk_size, backend=backend)
    engine = autorelease(MTDSEngine(cfg, dumb_metadata(fmt, kv_shape)))
...
def test_same_retrieve_store(fmt, backend, remote_serde, autorelease, lmserver_process):
    ...
    """ initialize the engine """
    cfg = MTDSEngineConfig.from_legacy(
...
def test_skipping(fmt, autorelease):
    ...
    engine1 = autorelease(MTDSEngine(cfg, dumb_metadata(fmt, kv_shape)))
    engine2 = autorelease(MTDSEngine(cfg, dumb_metadata(fmt, kv_shape)))
    """ store and persist """
...
def test_builder(autorelease):
    instance_id = "test"
    cfg = MTDSEngineConfig.from_legacy(chunk_size=256, persist_path="/tmp/a.txt")
    cfg2 = MTDSEngineConfig.from_legacy(chunk_size=512, persist_path="/tmp/a.txt")
    should_be_none = MTDSEngineBuilder.get(instance_id)
    assert should_be_none is None

    _engine = autorelease(
        MTDSEngineBuilder.get_or_create(instance_id, cfg, dumb_metadata())
    )
    _engine2 = autorelease(MTDSEngineBuilder.get(instance_id))  # noqa

    with pytest.raises(ValueError):
        MTDSEngineBuilder.get_or_create(instance_id, cfg2, dumb_metadata())


def test_env_configure():
    # Test the parsing of the configurations
    config = MTDSEngineConfig.from_defaults()
    for attr in fields(config):
        expected_env_name = f"MTDS_{attr.name.upper()}"
        os.environ[expected_env_name] = "0"
    newconfig = MTDSEngineConfig.from_env()
    for attr in fields(newconfig):
        ...
        if type(value) is int or type(value) is float:
            ...
            raise AssertionError("Unexpected type in MTDSEngineConfig")

    exclude_env_name = "MTDS_REMOTE_URL"
    del os.environ[exclude_env_name]

    newconfig2 = MTDSEngineConfig.from_env()
