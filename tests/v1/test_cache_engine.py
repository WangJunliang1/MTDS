...
def test_same_retrieve_store(autorelease_v1):
    ...
    cfg = MTDSEngineConfig.from_legacy(chunk_size=chunk_size, remote_url=None)

    engine = autorelease_v1(
        MTDSEngineBuilder.get_or_create(
    ...
    MTDSEngineBuilder.destroy("test")
...
def test_store_kv_tensors_mask(fmt, autorelease_v1):
    ...
    cfg = MTDSEngineConfig.from_legacy(chunk_size=chunk_size)
...
def test_builder(autorelease_v1):
    instance_id = "test"
    cfg = MTDSEngineConfig.from_legacy(chunk_size=256)
    cfg2 = MTDSEngineConfig.from_legacy(chunk_size=512)
    connector = None
    should_be_none = MTDSEngineBuilder.get(instance_id)
    assert should_be_none is None

    _engine = autorelease_v1(
        MTDSEngineBuilder.get_or_create(instance_id, cfg, dumb_metadata(), connector)
    )
    _engine2 = autorelease_v1(MTDSEngineBuilder.get(instance_id))  # noqa
