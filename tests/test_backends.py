...
from MTDS.storage_backend import CreateStorageBackend
from MTDS.storage_backend.hybrid_backend import LMCHybridBackend
from MTDS.storage_backend.local_backend import (
    LMCLocalBackend,
    LMCLocalDiskBackend,
)
from MTDS.storage_backend.remote_backend import LMCRemoteBackend
...
def get_config(t, remote_url):
    match t:
        case "local":
            return MTDSEngineConfig.from_defaults(
...
def test_creation(autorelease, lmserver_process):
    config_local = MTDSEngineConfig.from_defaults(
        local_device="cuda", remote_url=None
    )
    lmserver_url = lmserver_process.server_url
    config_remote = MTDSEngineConfig.from_defaults(
        local_device=None, remote_url=lmserver_url
    )
    config_hybrid = MTDSEngineConfig.from_defaults(
    ...
    config_fail = MTDSEngineConfig.from_defaults(local_device=None, remote_url=None)
...
def test_creation_from_file(autorelease, lmserver_process):
    ...
    config_local = MTDSEngineConfig.from_file(
        BASE_DIR / "data/test_creation_from_file/local.yaml"
    )
    config_disk = MTDSEngineConfig.from_file(
        BASE_DIR / "data/test_creation_from_file/disk.yaml"
    )
    config_remote = MTDSEngineConfig.from_file(
        BASE_DIR / "data/test_creation_from_file/remote.yaml"
    )
    config_hybrid = MTDSEngineConfig.from_file(
    ...
    config_fail = MTDSEngineConfig.from_file(
...
def test_restart(autorelease, lmserver_process):
    ...
    # MTDSEngineConfig.from_defaults(local_device = "cuda",
    # remote_url = None)
    kv_shape = (16, 2, 128, 4, 128)
    metadata = get_metadata(kv_shape=kv_shape)
    backend = autorelease(CreateStorageBackend(config, metadata))
