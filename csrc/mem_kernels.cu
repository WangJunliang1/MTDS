...
__global__ void load_and_reshape_multi_layer_kernel(
  ...
  for (int i = tid; i < scalars_per_token; i += num_threads) {
    ...
    if (DIRECTION)  // 1 is paged buffer to MTDS
      key_value[MTDS_offset] = paged_buffer_ptr[vllm_offset];
    else  // 0 is MTDS to paged buffer
      paged_buffer_ptr[vllm_offset] = key_value[MTDS_offset];
  }
}
 ...
 *
 * Param:
 *  - direction: false  means MTDS to PagedBuffer, true  means PagedBuffer to
 * MTDS
 */
...
void single_layer_kv_transfer(
    ...
    const bool direction,   // false: MTDS to PagedBuffer, true: PagedBuffer
                            // to MTDS
