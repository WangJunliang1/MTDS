...
inline __device__ void apply_token_rotary_embedding_fused(
  ...
  if (IS_NEOX) {
    ...
    old_cos = MTDS_LDG(old_cos_ptr + x_index);
    old_sin = MTDS_LDG(old_sin_ptr + x_index);

    new_cos = MTDS_LDG(new_cos_ptr + x_index);
    new_sin = MTDS_LDG(new_sin_ptr + x_index);
  } else {
    // GPT-J style rotary embedding.
    x_index = 2 * rot_offset;
    y_index = 2 * rot_offset + 1;
    old_cos = MTDS_LDG(old_cos_ptr + x_index / 2);
    old_sin = MTDS_LDG(old_sin_ptr + x_index / 2);

    new_cos = MTDS_LDG(new_cos_ptr + x_index / 2);
    new_sin = MTDS_LDG(new_sin_ptr + x_index / 2);
  }
